...
def runrforest(parte):
    ...
    def get_models():
        models = dict()
        samples = [None] + [k for k in np.arange(0.1, 0.99, 0.5)]  
        for i in samples:
            for j in ['auto','sqrt','log2',None]:
                n_trees = [i for i in range(1,51,10)]
                for n in n_trees:
                    for boot in [True, False]:
                        depths = [None] + [k for k in range(1,30,5)]
                        for m in depths:
                            splits = [2]
                            for split in splits:
                                leafs = [1]
                                for leaf in leafs:
                                    key = (i,j,n,m, boot, split,leaf)
    ...
    def RandomForest_Profile(df0):
        ...
        predictors = {
                        'all'    : weather+ macroeconomic,#, + self.entreprise
    ...
    tuning = RandomForest_Profile(df0)
    dict_table = pd.DataFrame.from_dict(tuning, orient='index',
                                        columns=['SKU','Params','Score']) 
    dict_table.to_csv('Parameters/Categoria_B/rforest/RandomForest%s.csv'%parte)
        
    print(time.time() - t0) 
    
    return  df0
    
runrforest(parte=sys.argv[1])
